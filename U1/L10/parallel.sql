...
--task1
set autotrace on;
SELECT  * FROM times WHERE day_name = 'Wednesday' AND calendar_quarter_number IN (2,4);

set autotrace on;
SELECT  /*+ PARALLEL(4) */ * FROM times WHERE day_name = 'Wednesday' AND calendar_quarter_number IN (2,4);


--task2
delete from times where day_name = 'Tuesday';

delete /*+ parallel enable_parallel_dml */ from times where day_name = 'Tuesday';
...
CREATE TABLE times parallel 4 AS
SELECT 
  TRUNC( sd + rn ) time_id,
  TO_CHAR( sd + rn, 'fmDay' ) day_name,
  TO_CHAR( sd + rn, 'D' ) day_number_in_week,
